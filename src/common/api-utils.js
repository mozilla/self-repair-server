...
// all of these are copied from Mozilla Addon-SDk
...
exports.validateOptions = function validateOptions(options, requirements) {
  ...
  for (let key in requirements) {
    ...
    let __okv  = (key in options) ?
               [options[key], true] :
               [undefined, false];

    let optsVal = __okv[0];
    let keyInOpts =  __okv[1];
    if (req.map) {
      try {
    ...
    if (req.is) {
      ...
      if (isArray(types)) {
        isOptional = ['undefined', 'null'].every(function (v) {return ~types.indexOf(v)});
...
let isTruthyType = function (type) {return !(type === 'undefined' || type === 'null')};
let findTypes = function (v) { while (!isArray(v) && v.is) v = v.is; return v };
